...
interface Goal { id: string; text: string; shortLabel: string }
...
export function RetrospectiveView({ matchId, playerId, selectedGoals }: RetroViewProps): React.JSX.Element {
      ...
      try {
        ...
        startSubmitTransition(async () => {
          const result = await submitRetrospective({
            matchId, playerId,
            goal1Id: g1.id, goal1Result: goalResults[g1.id] ?? 'nein', goal1Note: goalNotes[g1.id] ?? '',
            goal2Id: g2.id, goal2Result: goalResults[g2.id] ?? 'nein', goal2Note: goalNotes[g2.id] ?? '',
            goal3Id: g3.id, goal3Result: goalResults[g3.id] ?? 'nein', goal3Note: goalNotes[g3.id] ?? '',
            strongestLever, missedSignalNote: missedSignal, nextTestNote: nextTest,
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-navy-900">Nachbereitung</h1>
        ...
        <div className="space-y-4">
          {selectedGoals.map((goal) => (
            <div key={goal.id} className="rounded-lg border border-slate-200 bg-white p-4">
              ...
              <div className="mb-2 flex gap-2" role="group" aria-label={`Ergebnis für ${goal.shortLabel}`}>
      ...
      <section aria-label="Analyse-Qualität">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-slate-500">Analyse-Qualität</h2>
        <div className="space-y-4 rounded-lg border border-slate-200 bg-white p-4">
          <div>
            <p className="mb-2 text-sm font-medium text-slate-700">Welcher Cluster war der stärkste Hebel?</p>
              ...
              {LEVER_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`flex-1 rounded-md border py-2 text-sm font-semibold ${
                    strongestLever === opt.value ? 'border-navy-900 bg-navy-900 text-white' : 'border-slate-200 text-slate-600'
                  }`}
                >
                  {opt.label}
                </button>
          ...
          <div>
            <label htmlFor="missed-signal" className="mb-1 block text-sm font-medium text-slate-700">
              Wo habe ich ein Signal gesehen und nicht genutzt? (optional)
            </label>
            <input id="missed-signal" type="text" value={missedSignal} onChange={(e) => setMissedSignal(e.target.value)}
              className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm" />
          </div>
          <div>
            ...
            <input id="next-test" type="text" value={nextTest} onChange={(e) => setNextTest(e.target.value)}
              className="w-full rounded-md border border-slate-200 px-3 py-2 text-sm" />
          </div>
        </div>
      </section>
